...
# ============================================================
# MEXC FUTURES API & YEDEK KAYNAK AYARLARI
# ============================================================
...
def get_klines(symbol):
    # 1. Aşama: Binance Alternatif URL'leri denenir[span_8](start_span)[span_8](end_span)
    binance_symbol = symbol.replace("_", "")
    for base_url in BASE_URLS:
        url = f"{base_url}?symbol={binance_symbol}&interval={TIMEFRAME}&limit={LIMIT}"
        data = http_get_json(url)
        if data and isinstance(data, list) and len(data) > 0:
            # Binance formatını standart sözlüğe çeviriyoruz
            formatted = {"close": [], "high": [], "low": [], "vol": []}
            for row in data:
                formatted["close"].append(row[4])
                formatted["high"].append(row[2])
                formatted["low"].append(row[3])
                formatted["vol"].append(row[5])
            return formatted

    # 2. Aşama: MEXC Futures API denenir
    url = f"{MEXC_BASE_URL}/{symbol}?interval=Min15"
    data = http_get_json(url)
    if data and data.get("success") and "data" in data:
        return data["data"]

    # 3. Aşama: MEXC Spot Yedek API[span_9](start_span)[span_9](end_span)
    try:
        mexc_symbol = symbol.replace("USDT", "_USDT")
        mexc_url = f"https://api.mexc.com/api/v3/klines?symbol={mexc_symbol}&interval=15m&limit={LIMIT}"
        mexc_data = http_get_json(mexc_url)
        if mexc_data and isinstance(mexc_data, list) and len(mexc_data) > 0:
            formatted = {"close": [], "high": [], "low": [], "vol": []}
            for row in mexc_data:
                formatted["close"].append(row[4])
                formatted["high"].append(row[2])
                formatted["low"].append(row[3])
                formatted["vol"].append(row[5])
...
# --- SUNUCU VE SAĞLIK KONTROLÜ (HEALTH-CHECK) ---
class HealthCheckHandler(BaseHTTPRequestHandler):
    ...
    def do_HEAD(self):  # Render ve Uptime servisleri için eklendi[span_10](start_span)[span_10](end_span)[span_11](start_span)[span_11](end_span)
        self.send_response(200)
        self.end_headers()
...
def run_health_check_server():
    port = int(os.getenv("PORT", 8080))  # Render PORT değişkeni uyumlu[span_12](start_span)[span_12](end_span)
